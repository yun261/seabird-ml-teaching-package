...
def generate_condition_list(issue, ex, dataset, model_name):
    ...
    common_base_dir = os.path.join(PROJECT_ROOT, "data", "model-output")
    path0 = f"{issue}/{ex}"
    path1 = f"{dataset}"
    path2 = f"{model_name}"
    path3 = f"*"
...
def generate_test_config_target(issue, ex, dataset, model_name, condition, seed):
    ...
    if not glob.glob(config_target, recursive=True):
        config_target = os.path.join(common_base_dir, path0, path1, path2, path3, path5)
...
def generate_results_save_path(path_list, checkpoints_fname=None):
    ...
    results_save_roodir = os.path.join(PROJECT_ROOT, "data", "test-results")
...
def load_and_setup_test_config(config_path, TEST_CUDA_ID, checkpoints_fname=None):
    ...
    OmegaConf.update(cfg, "path.nas.rootdir", PROJECT_ROOT, merge=True)
...
def setup_test_loader(cfg):
    from src.data_module import prep_dataloaders_for_supervised_learning
    (
        _, _, test_loader
