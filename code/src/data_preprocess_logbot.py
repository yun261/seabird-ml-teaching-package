'''
Data Pre-processing code for Logbot (bio-logger with AI) dataset.
...
# --------------------------------------------------
# Convert Raw CSV files to Preprocessed CSV files
# --------------------------------------------------

def get_raw_date_information(raw_data_path, animal_id_path):
    '''Extract meta data (information) of raw data
    Args:
    ...
    df_animal_id = pd.read_csv(animal_id_path, low_memory=False)    #关闭低内存模式，让 pandas 一次性读取所有数据
...
def read_raw_data_and_refine_timestamp(raw_data_path, correct_timestamp):
    ...
    # Refine timestamp data if needed.
...
def divide_df_if_timestamp_gap_detected(
    df, 
    acc_sampling_rate, 
    gap_min_limit = 5 # 5 -> 125 min (2 hours + 5 min)
):
    ''' Divide data frame before resampling
        ( Otherwise, it takes a lot of time for resampling. )
    Args:
        df (DataFrame): a raw data
        acc_sampling_rate (int): original sampling rate of sensor data
        gap_min_limit (int): SAMPLING_RATE * GAP_SEC_LIMIT: # gap_min_limit = 5: 25 * 60 * 5 = 7500 sec (125 min)
            default 5 -> 125 min (2 hours + 5 min)
    ...
    GAP_SEC_LIMIT = 60 * gap_min_limit
    ...
    for i in range(1, len(df)):
        ...
        # if diff > GAP_SEC_LIMIT: # gap_min_limit = 5: 60 * 5 = 300 sec (5 min)
...
def resampling(df, intermediate_sampling_rate=1000, output_sampling_rate=25):
    '''
    Resampling data: 31Hz -> 1000 Hz -> 25 Hz
    
    Args:
        df (DataFrame): a data frame of sensor data with original sampling rate
        intermediate_sampling_rate (int): default value = 1000 (1000 Hz)
...
def run_resampling_and_concat_df(df_list, acc_sampling_rate, remove_sec=3, check_df=False):
    ...
    INTERMEDIATE_SAMPLING_RATE = 1000
...
# Note: this function was originally prepared 
# for calibration of attachment position difference
# but this was not used for dl-wabc
def calibrate_mounting_position_stomach(df):
    '''
    acc_x -> acc_x
    acc_y -> acc_y
    acc_z -> -1 * acc_z
    See Korpela et al., 2020 Figure 1a
    https://www.nature.com/articles/s42003-020-01356-8
    '''
    df['acc_z'] = df['acc_z'] * -1
...
def save_preprocessed_data(df, output_dir_path, species, animal_id):
    '''
    Args:
    ...
    if len(df) == 0:
        ...
        if species == "omizunagidori":
            label_id_path = os.path.join(_src_dir, "..", "data", "datasets", "id_files", "label_id_omizunagidori.csv")
        elif species == "umineko":
            label_id_path = os.path.join(_src_dir, "..", "data", "datasets", "id_files", "label_id_umineko.csv")
        ...
        save_dir = output_dir_path + species + "/"
        if os.path.exists(save_dir) == False:
            os.makedirs(save_dir)

        df_save_path = save_dir + str(animal_id) + ".csv"
...
# -------------------------------------------------------------
# Convert preprocessed data (.csv) files to npy or npz files
# -------------------------------------------------------------
...
def extract_sliding_windows(preprocessed_data_path, 
    ...
    Args:
        ...
        sliding_windo_size (int): number of data points in a time-window = window size
...
def extract_sliding_windows_v2(
    ...
    # for i in tqdm(range(0, len(df_values)-window_size, window_step_size)):
        ...
        is_X_ok = check_X_in_a_window(X_tmp)
...
def check_X_in_a_window(X):
    """
    Return True if X 
    
    1. does not have na
    2. does not have many zeros
    
    Args:
        X (np.ndarray): acc_x, acc_y, acc_z
    Return:
        is_X_ok (bool):
    """
    ...
    num_na_in_X = np.sum(np.isnan(X))
    X_has_na = True if num_na_in_X > 0 else False
    
    num_zeros_in_X = np.sum(X == 0)
    X_has_many_zeros = True if num_zeros_in_X >= 5 else False
    
    if X_has_na == True or X_has_many_zeros == True: 
        is_X_ok = False
    else:
        is_X_ok = True
...
def check_npz_file(npz_file_path):
    print("----------")
    npz = np.load(npz_file_path, allow_pickle=True)
    timestamp = npz["timestamp"] # unixtime
    domain = npz["animal_id"]
    label = npz["label_id"]
    X = npz["X"]
    print(
        "animal_id (domain):", domain, " | timestamp shape:", timestamp.shape,\
        " | label shape:", label.shape, " | X shape:", X.shape)

    # check timestamp
    start_timestamp = timestamp[:, 0][0]
    end_timestamp = timestamp[:, -1][0]
    time_diff = end_timestamp - start_timestamp
    if time_diff >= 2.0: # Return True if there is a gap of more than 2 seconds
        print("time_diff: ", round(time_diff, 4), " -> Timestamp gap detected!")
        timestamp_gap = True
    else:
        print("time_diff: ", round(time_diff, 4), " -> No timestamp gap") 
        timestamp_gap = False

    # check label
    if np.sum(np.isnan(label)) == 0:
        print("All instances are labelled")
        labelled = True
    else:
        print("NA found in label!")
        labelled = False

    return X, label, domain, timestamp